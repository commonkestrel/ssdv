...
fn main() -> ExitCode {
    ...
    in_file.read_to_end(&image).expect("Unable to read from file");

    let encoder = ssdv::Encoder::new(
        CALLSIGN,
        IMAGE_ID,
        QUALITY,
        image,
    ...
    for (i, chunk) in encoder.enumerate() {
        match chunk {
            Ok(c) => out_file.write_all(&chunk).expect("Unable to write to output file"),
